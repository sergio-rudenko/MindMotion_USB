...
#define Send0LengthData()   if(!(_GetUSB_CTRL0()&EP0_CTRL_TRANEN)) _SetUSB_CTRL0(EP0_CTRL_TRANEN|0);
...
u8* Standard_GetStatus(u16 Length)
{
  ...
  if (Type_Recipient == (STANDARD_REQUEST | ENDPOINT_RECIPIENT))
  {
    u8 Related_Endpoint;
...
RESULT Standard_ClearFeature(void)
{
  ...
  if (Type_Rec == (STANDARD_REQUEST | ENDPOINT_RECIPIENT))
  {
...
void Data_Setup0(void)
{
  ...
  if (Request_No == GET_DESCRIPTOR)
  {
    if (Type_Recipient == (STANDARD_REQUEST | DEVICE_RECIPIENT))
    {
      ...
      if (wValue1 == CONFIG_DESCRIPTOR)
      {
        CopyRoutine = pProperty->GetConfigDescriptor;
      }
      else
      if (wValue1 == STRING_DESCRIPTOR)
      {
  ...
  if (ValBit(pInformation->USBbmRequestType, 7))
  {
    ...
    if (pInformation->Ctrl_Info.Usb_wLength < pInformation->USBwLength)
    {
...
u8 In0_Process(void)
{
  ...
  if (ControlState == WAIT_STATUS_IN)
  {
